...
class StratumSession(RPCSession):
    ...
    async def handle_subscribe(self, *args):
        ...
        # Assign unique extranonce1 per session (fixed per connection)
        self.extranonce1 = self._state.bits_counter.to_bytes(4, 'big').hex()
        self._state.bits_counter += 1

        extranonce2_size = 4  # ccminer expects this field

        # Return subscribe response (protocol spec)
        return [
    ...
    async def handle_submit(self, worker: str, job_id: str, nonce_hex: str, extranonce2_hex: str):
        now = datetime.now()
        ...
        # --- NEW: rebuild coinbase with extranonce1 + extranonce2 ---
        coinbase1_hex, coinbase2_hex = split_coinbase_for_extranonce(state.coinbase_tx, state.height)
        full_coinbase_hex = coinbase1_hex + self.extranonce1 + extranonce2_hex + coinbase2_hex
        state.coinbase_tx = bytes.fromhex(full_coinbase_hex)

        # Recompute coinbase txid and merkle root
        state.coinbase_txid = dsha256(state.coinbase_tx)  # strip witness if needed
        txids = [state.coinbase_txid] + [bytes.fromhex(tx)[::-1] for tx in state.externalTxs]
        merkle = merkle_from_txids(txids)

       # Rebuild block header with new merkle
        state.header = (
         state.version.to_bytes(4, 'little') +
         state.prevHash +
         merkle +
         state.timestamp.to_bytes(4, 'little') +
         bytes.fromhex(state.bits)[::-1] +
...
def target_to_difficulty(target: int) -> float:
    return SOTERG_DIFF1_TARGET / target
# -----------------------------
# Coinbase split helper
# -----------------------------
def split_coinbase_for_extranonce(coinbase_tx: bytes, height: int) -> tuple[str, str]:
    """
    Split coinbase into coinbase1 and coinbase2 at the extranonce insertion point.
    coinbase1 ends right after the BIP34 height push.
    coinbase2 starts immediately after, where extranonce will be inserted.
    """
    tx_hex = coinbase_tx.hex()

    # Encode block height (BIP34) as little-endian
    height_bytes = height.to_bytes((height.bit_length() + 7) // 8, 'little')
    height_push = op_push(len(height_bytes)).hex() + height_bytes.hex()

    # Find where the height push occurs in the coinbase
    idx = tx_hex.find(height_push)
    if idx == -1:
        raise ValueError("Could not locate BIP34 height push in coinbase")

    # Split immediately after the height push
    split_at = idx + len(height_push)
    coinbase1_hex = tx_hex[:split_at]
    coinbase2_hex = tx_hex[split_at:]

    return coinbase1_hex, coinbase2_hex
# -----------------------------
# Subscription handler
# -----------------------------
async def handle_subscribe(session: StratumSession):
    # Assign unique extranonce1 per session
    extranonce1 = session.id.to_bytes(4, 'big').hex()
    extranonce_size = 4
    await session.send_notification("mining.set_extranonce", (extranonce1, extranonce_size))
...
async def notify_x12rt(session: StratumSession, state: TemplateState):
    ...
    prevhash_hex = state.prevHash[::-1].hex()

    # Proper coinbase split using helper
    coinbase1_hex, coinbase2_hex = split_coinbase_for_extranonce(state.coinbase_tx, state.height)
    ...
    target_int = bits_to_target(state.bits)
    difficulty_value = target_to_difficulty(target_int)

    # Debug
    print("=== Job Debug ===")
    print(f"Job ID: {job_id}")
    print(f"Prevhash: {prevhash_hex}")
    print(f"Coinbase1: {coinbase1_hex}")
    print(f"Coinbase2: {coinbase2_hex}")
    print(f"Extranonce1 (session): {session.extranonce1}")
    print(f"Version: {version_hex}, Bits: {nbits_hex}, Time: {ntime_hex}")
    print(f"Target: {hex(target_int)}, Difficulty: {difficulty_value}")
    print("=================")

    # Send difficulty
    await session.send_notification("mining.set_difficulty", (difficulty_value,))
