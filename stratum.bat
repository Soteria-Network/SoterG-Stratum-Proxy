...
if NOT "%CURRENT_DIRECTORY:~-33%" == "\soterg-stratum-proxy\windows\" (
    if NOT "%CURRENT_DIRECTORY:~-40%" == "\soterg-stratum-proxy-master\windows\" (
        echo Error: Please run this batch file as-is from its original location in the soterg-stratum-proxy folder
	pause
        exit /B
    )
)
...
set /p "ALLOW_EXTERNAL_CONNECTIONS_INPUT=Allow miners running externally from this computer to connect to this stratum? y/n (Default n): "
...
set /p "STRATUM_PORT=What port would you like to run the stratum on?: "
if "%STRATUM_PORT%" == "" (
    echo You must input a port.
    goto PRE_CHECK_PORT
)

set /a "TEST_PORT=%STRATUM_PORT%+0"
if %TEST_PORT% LEQ 1024 (
    echo Not a valid port: %STRATUM_PORT%
	set "STRATUM_PORT="
    goto PRE_CHECK_PORT
)
...
echo echo Connect to your stratum proxy (with a miner on this computer) at stratum+tcp://localhost:%STRATUM_PORT%>>"%FILE_LOCATION%"
echo echo ==========================================================>>"%FILE_LOCATION%"
echo Running SoterG Stratum Proxy on %IS_MAINNET_INPUT% network
echo "%CURRENT_DIRECTORY%python_files\python.exe" "%CURRENT_DIRECTORY%..\soterg-stratum-proxy.py" %STRATUM_PORT% %NODE_IP% %RPC_USERNAME% %RPC_PASSWORD% %NODE_PORT% %EXTERNAL_STRING_VALUE% %TESTNET_STRING_VALUE%>>"%FILE_LOCATION%"
